...
const subscriptionPlanHandler = {
  ...
  getSubscriptionPlanDetail: async (data: any) => {
    ...
    return showResponse(true, subs_msg, plan, statusCodes.SUCCESS);
  },
  stripeSubscribePlan: async (request: any, userId: string) => {
  ...
  if (!plan) {
    return showResponse(false, "Subscription plan not found", null, statusCodes.NOT_FOUND);
  }
  ...
  if (existingSub) {
    return showResponse(false, "You have already subscribed to this plan", null, statusCodes.API_ERROR);
  }
  ...
  await userAuthModel.findByIdAndUpdate(userId, {
    is_subscribed: true,
  });

  return showResponse(true, "Subscription successful", savedSub, statusCodes.SUCCESS);
}
};
